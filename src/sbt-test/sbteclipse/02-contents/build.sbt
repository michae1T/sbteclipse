...
TaskKey[Unit]("verify-classpath-xml-root") <<= baseDirectory map { dir =>
  ...
  if (!(classpath.child contains <classpathentry kind="output" path="target/eclipse/classes"/>))
    error("""Expected .classpath of root project to contain <classpathentry kind="output" path="target/eclipse/classes"/> """)
}
